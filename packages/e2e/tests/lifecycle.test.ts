...
describeOnValidator("full lifecycle (create → mint → trade → settle → redeem)", function () {
  ...
  it("runs the whole cycle and holds every invariant", async () => {
    ...
    expect(book!.asks.filter((o) => o.active).length, "one resting ask").to.equal(1);
    ...
    expect(takerPos.yes.toString(), "taker bought 3 Yes").to.equal(tradeSize.toString());
    // Taker paid 3 × $0.60 = $1.80.
    expect((takerUsdcBefore - takerPos.usdc).toString(), "taker paid 3×$0.60").to.equal(
      (BigInt(askPrice.toString()) * 3n).toString()
    );
    ...
    expect(after.settlementPrice!.toString(), "settlement price unchanged").to.equal(
      String(STRIKE + 1)
    );
    ...
    expect((takerUsdcPost - takerUsdcPre).toString(), "winner paid $3.00").to.equal(
    ...
    expect(makerPosBefore.yes.toString(), "maker kept 2 Yes").to.equal(ONE.muln(2).toString());
    ...
    expect((await fx.vaultBalance(market)).toString(), "vault drained").to.equal("0");
    await fx.assertCollateralization(market);
    ...
    expect(finalMarket.winningRedeemed.toString(), "total paid == $5").to.equal(
      (BigInt(PAYOFF_UNIT.toString()) * 5n).toString()
    );
  });
});
